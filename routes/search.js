...
SELECT a.*, b.country_short, b.country_origin, c.state_short, c.state_origin FROM running_group AS a, country AS b, state AS c WHERE a.country_id = b.id AND a.state_id = c.id AND c.state_short = \"%s\" \
    ...
    //searchQueryString = "SELECT * FROM search_index WHERE MATCH(group_name) AGAINST('%s' IN BOOLEAN MODE) OR MATCH(country) AGAINST('%s' IN BOOLEAN MODE) OR MATCH(state) AGAINST('%s' IN BOOLEAN MODE) OR MATCH(county) AGAINST('%s' IN BOOLEAN MODE) OR MATCH(city) AGAINST('%s' IN BOOLEAN MODE) ORDER BY id DESC LIMIT %s, 10";
    /*searchQueryString = "SELECT y.*, z.cover_photo, z.lat, z.lng FROM running_group AS z, \
(SELECT * FROM search_index WHERE MATCH(group_name, country, state, county, city, address) AGAINST(\"*%s*\" IN BOOLEAN MODE) ORDER BY id DESC) AS y \
WHERE z.id = y.id LIMIT %s, 10",*/
	searchQueryString = "SELECT y.*, z.cover_photo, z.lat, z.lng, b.country_short, c.state_short FROM \
running_group AS z JOIN country AS b ON z.country_id = b.id JOIN state AS c ON  z.state_id = c.id, \
...
WHERE z.id = y.id",
	fullSearchQueryString = "SELECT y.*, z.cover_photo, z.lat, z.lng, b.country_short, c.state_short FROM \
running_group AS z JOIN country AS b ON z.country_id = b.id JOIN state AS c ON  z.state_id = c.id, \
	(SELECT * FROM search_index WHERE group_name LIKE \"%%%s%%\" \
	ORDER BY id DESC) AS y \
WHERE z.id = y.id",
	searchWithDayOrTimeQueryString = "SELECT i.* FROM (SELECT y.*, z.cover_photo, z.lat, z.lng, b.country_short, c.state_short FROM \
running_group AS z JOIN country AS b ON z.country_id = b.id JOIN state AS c ON  z.state_id = c.id, \
	(SELECT * FROM search_index WHERE \
	MATCH(group_name, country, state, county, city, address) AGAINST(\"%s*\" IN BOOLEAN MODE) OR \
	MATCH(group_name, country, state, county, city, address) AGAINST(\"%s\") \
...
WHERE z.id = y.id %s) AS i JOIN group_schedule AS j ON i.id = j.group_id",
	fullSearchWithDayOrTimeQueryString = "SELECT i.* FROM (SELECT y.*, z.cover_photo, z.lat, z.lng, b.country_short, c.state_short FROM \
running_group AS z JOIN country AS b ON z.country_id = b.id JOIN state AS c ON  z.state_id = c.id, \
	(SELECT * FROM search_index WHERE group_name LIKE \"%%%s%%\" \
	ORDER BY id DESC) AS y \
WHERE z.id = y.id %s) AS i JOIN group_schedule AS j ON i.id = j.group_id",
	regionQueryString = " AND b.country_short = \"%s\" AND c.state_short = \"%s\"",
	/*searchInARegionQueryString = "SELECT y.*, z.cover_photo, z.lat, z.lng, b.country_short, c.state_short FROM \
running_group AS z JOIN country AS b ON z.country_id = b.id JOIN state AS c ON  z.state_id = c.id, \
(SELECT * FROM search_index WHERE MATCH(group_name, country, state, county, city, address) AGAINST(\"*%s*\" IN BOOLEAN MODE) ORDER BY id DESC LIMIT %s, 10) AS y \
WHERE z.id = y.id AND b.country_short = \"%s\" AND c.state_short = \"%s\"",*/
	searchInARegionQueryString = "SELECT y.*, z.cover_photo, z.lat, z.lng, b.country_short, c.state_short FROM \
running_group AS z JOIN country AS b ON z.country_id = b.id JOIN state AS c ON  z.state_id = c.id, \
	(SELECT * FROM search_index WHERE \
	MATCH(group_name, country, state, county, city, address) AGAINST(\"%s*\" IN BOOLEAN MODE) OR \
	...
	ORDER BY id DESC) AS y \
WHERE z.id = y.id  AND b.country_short = \"%s\" AND c.state_short = \"%s\"",
	fullSearchInARegionQueryString = "SELECT y.*, z.cover_photo, z.lat, z.lng, b.country_short, c.state_short FROM \
running_group AS z JOIN country AS b ON z.country_id = b.id JOIN state AS c ON  z.state_id = c.id, \
	(SELECT * FROM search_index WHERE group_name LIKE \"%%%s%%\" \
	ORDER BY id DESC) AS y \
WHERE z.id = y.id  AND b.country_short = \"%s\" AND c.state_short = \"%s\"",
	searchInARegionWithDayOrTimeQueryString = "SELECT i.* FROM (SELECT y.*, z.cover_photo, z.lat, z.lng, b.country_short, c.state_short FROM \
running_group AS z JOIN country AS b ON z.country_id = b.id JOIN state AS c ON  z.state_id = c.id, \
	(SELECT * FROM search_index WHERE \
	MATCH(group_name, country, state, county, city, address) AGAINST(\"%s*\" IN BOOLEAN MODE) \
	MATCH(group_name, country, state, county, city, address) AGAINST(\"%s\") \
	ORDER BY id DESC) AS y \
WHERE z.id = y.id AND b.country_short = \"%s\" AND c.state_short = \"%s\") AS i JOIN group_schedule AS j ON i.id = j.group_id",
	fullSearchInARegionWithDayOrTimeQueryString = "SELECT i.* FROM (SELECT y.*, z.cover_photo, z.lat, z.lng, b.country_short, c.state_short FROM \
running_group AS z JOIN country AS b ON z.country_id = b.id JOIN state AS c ON  z.state_id = c.id, \
	(SELECT * FROM search_index WHERE group_name LIKE \"%%%s%%\" \
	ORDER BY id DESC) AS y \
WHERE z.id = y.id AND b.country_short = \"%s\" AND c.state_short = \"%s\") AS i JOIN group_schedule AS j ON i.id = j.group_id",
...
router.get('/', function (req, res, next) {
	...
	var queryString = getSearchQueryString(query, day, time, offset, false);//util.format(searchQueryString, query, query) + util.format(limit, offset);
...
function getBaseSearchQueryString (query, day, time, region, offset, is_full) {
	var queryString;
	if (day || time) {
		if (is_full) {
			queryString = util.format(fullSearchWithDayOrTimeQueryString, query, region);
		} else {
			queryString = util.format(searchWithDayOrTimeQueryString, query, query, region);
		}
	} else {
		if (is_full) {
			queryString = util.format(fullSearchQueryString, query, region);
		} else {
			queryString = util.format(searchQueryString, query, query, region);
		}
	}
	return queryString;
}
...
function getSearchWithDayTimeQueryString (queryString, day, time) {
	...
	if (time) {
		...
		if (time.length === 1) {
			timeQueryString += (getTimeQuery(time));
		} else if (time.length === 2) {
			timeQueryString += ( " ( " + getTimeQuery(time[0]) + " OR " + getTimeQuery(time[1]) + " )");
		}
...
function getSearchInARegionQueryString (query, offset, day, time, country_short, state_short, is_full) {
	...
	var queryString = getBaseSearchQueryString(query, day, time, region, offset, is_full);
	queryString = getSearchWithDayTimeQueryString(queryString, day, time);
	queryString = getSearchLimitQueryString(queryString, offset);
	return queryString;
}
